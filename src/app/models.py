...
class Recipe(db.Model):
    ...
    recipe_type_code = db.Column(
        db.Integer, db.ForeignKey('recipe_types.code'))
    recipe_type = db.relationship(
        'RecipeType', foreign_keys=[recipe_type_code])
    description = db.Column(db.String(256), nullable=False)
...
class MysticBurger(db.Model):
    ...
    def __str__(self):
        return f'{self.store}, {self.category}, {self.item},\
        {self.description}, {self.price}, {self.qty}, {self.magic}'
